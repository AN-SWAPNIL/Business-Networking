...
export class MatchingAlgorithm {
  ...
  private calculateMatch(currentUser: User, targetUser: User): Match {
    ...
    const preferenceScore = this.calculatePreferenceCompatibility(currentUser, targetUser, matchReasons)
    score += preferenceScore * 0.4

    // 2. Location Proximity (20% weight)
    const locationScore = this.calculateLocationCompatibility(currentUser, targetUser, matchReasons)
    score += locationScore * 0.2

    // 3. Shared Interests (20% weight)
    const interestScore = this.calculateInterestCompatibility(currentUser, targetUser, sharedInterests, matchReasons)
    score += interestScore * 0.2

    // 4. Complementary Skills (10% weight)
    const skillScore = this.calculateSkillCompatibility(currentUser, targetUser, complementarySkills, matchReasons)
    score += skillScore * 0.1

    // 5. Company/Industry Alignment (10% weight)
    const companyScore = this.calculateCompanyCompatibility(currentUser, targetUser, matchReasons)
  ...
  private calculatePreferenceCompatibility(currentUser: User, targetUser: User, reasons: string[]): number {
    let score = 0
    let matches = 0
    ...
    if (currentUser.preferences.collaborate && targetUser.preferences.collaborate) {
      score += 20
      matches++
  ...
  private calculateLocationCompatibility(currentUser: User, targetUser: User, reasons: string[]): number {
  ...
  private calculateInterestCompatibility(
    ...
    const shared = targetUser.interests.filter((interest) => currentInterests.has(interest))
    ...
    const score = Math.min(
      (shared.length / Math.max(currentUser.interests.length, targetUser.interests.length)) * 100,
      100,
    )
  ...
  private calculateSkillCompatibility(
    ...
    const complementary = targetUser.skills.filter((skill) => !currentSkills.has(skill))

    complementarySkills.push(...complementary.slice(0, 3))

    if (complementary.length > 0) {
      reasons.push(`Complementary skills in ${complementary.slice(0, 2).join(", ")}`)
      return Math.min((complementary.length / targetUser.skills.length) * 100, 100)
    }

    return 20
  }

  private calculateCompanyCompatibility(currentUser: User, targetUser: User, reasons: string[]): number {
  ...
  private getCompanyType(companyName: string): string {
    ...
    if (name.includes("tech") || name.includes("software") || name.includes("startup")) return "tech"
    if (name.includes("design") || name.includes("creative") || name.includes("agency")) return "design"
    if (name.includes("consulting") || name.includes("advisory") || name.includes("partners")) return "consulting"
    if (name.includes("capital") || name.includes("ventures") || name.includes("investment")) return "investment"
    return "other"
  }
}
