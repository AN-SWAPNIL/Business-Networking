...
interface Match {
  ...
  matchTypes: string[]; // AI-determined match types
}
...
export function MatchCard({ match, currentUser }: MatchCardProps) {
  ...
  // Use AI-determined match types instead of manual logic
  const getPrimaryMatchType = () => {
    return matchTypes && matchTypes.length > 0 ? matchTypes[0] : "Professional";
  };
    ...
    <Card className="hover:shadow-lg transition-all duration-200 border-2 hover:border-primary/20">
      <CardHeader className="text-center pb-4">
        <div className="relative">
          ...
          >
            <span
              className={`text-xs font-bold ${getCompatibilityColor(
                compatibilityScore
              )}`}
            >
              {compatibilityScore}%
